...
const Table = ({ data, maxHeight, textSize }: TableProps) => {
  const [tableData, setTableData] = useState<OverviewDeploymentTrackData[]>(
    data
  );
  ...
  const formatColVal = (
    colObj: OverviewDeploymentTrackData,
    colKey: string
  ) => {
      ...
      case "time_start":
        return value
          ? new Date(value.toString()).toLocaleString(
              "de-DE",
              DateTimeLocaleOptions
            )
          : "no data";
      case "time_end":
        const converted = convert(
          new Date(value).getTime() -
            new Date(Object(colObj)["time_start"]).getTime(),
          "ms"
        ).to("best");
  ...
  const sort = (direction: string, column_key: string) => {
    const type = typeof data.find(
      (item) => item[column_key as keyof OverviewDeploymentTrackData] !== null
  ...
  const createColumn = (
    index: number,
    i: number,
    row: OverviewDeploymentTrackData,
    titleKey: string
  ) => {
    if (titleKey === "deployment_id") {
      return (
        <div
          onClick={() => {
            dataStore.setSelectedColumn(
              Object(row)["logger_id"],
              Object(row)[titleKey]
            );
  ...
  return (
    <div className="overflow-x-auto rounded-t-lg shadow-md">
      ...
      {tableData && tableData?.length > 0 && (
        <div
          className={`grid grid-rows-1 bg-white gap-1 min-w-[800px] ${
        ...
        >
          ...
          <div className={`bg-white ${maxHeight}`}>
            {tableData?.map((row, index) => (
              ...
              >
                <div
                  className={`grid grid-cols-7 gap-0.5 bg-white ${maxHeight}`}
                >
